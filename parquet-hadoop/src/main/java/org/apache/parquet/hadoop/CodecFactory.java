...
public class CodecFactory implements CompressionCodecFactory {
  ...
  /** Smallest output buffer tried by {@link #decompressUnknownSize}. */
  private static final int MIN_UNKNOWN_SIZE_BUFFER = 8 * 1024;

  /**
   * Largest output buffer tried by {@link #decompressUnknownSize}. Java arrays are indexed by int,
   * and some JVMs reserve a few header words, so this stays just below {@link Integer#MAX_VALUE}.
   */
  private static final int MAX_UNKNOWN_SIZE_BUFFER = Integer.MAX_VALUE - 8;

  /**
   * Decompresses a complete compression block whose decompressed size is not known in advance. This
   * is used to resolve FILE self-references, whose stored representation records only the size of
   * the (compressed) stored block and not the size of the resolved bytes. Each self-reference is an
   * independent compression block, so the entire {@code compressed} range is supplied to the codec
   * in one shot.
   *
   * <p>All codecs are supported, including those that record no decompressed size of their own. The
   * format spec allows a reader to "decompress into a dynamically sized buffer", which is what this
   * does: it guesses an output size, and whenever the codec fills the buffer exactly — the signal
   * that the output may have been cut off — it doubles the guess and retries. Retries are bounded by
   * the 2 GiB ceiling on a Java array.
   *
   * <p>The {@link Decompressor} is driven directly rather than through
   * {@link BytesDecompressor#decompress(BytesInput, int)} or a stream-drain loop. The former reads
   * back exactly the requested number of bytes and so cannot report a short read, and Parquet's
   * codec streams are deliberately unframed ({@code NonBlockedDecompressorStream}), signalling a
   * fully consumed block by throwing rather than by returning end-of-input.
   *
  ...
  public BytesInput decompressUnknownSize(CompressionCodecName codecName, BytesInput compressed)
    ...
    if (compressedBytes.length == 0) {
      // An empty payload compresses to nothing and resolves back to nothing.
    ...
    try {
      ...
      // the first attempt usually suffices.
      long attemptSize = Math.max((long) compressedBytes.length * 2, MIN_UNKNOWN_SIZE_BUFFER);
      while (true) {
        int candidate = (int) Math.min(attemptSize, MAX_UNKNOWN_SIZE_BUFFER);
        boolean lastAttempt = candidate == MAX_UNKNOWN_SIZE_BUFFER;
        byte[] output = new byte[candidate];
        int total = 0;
        ...
        try {
          while (total < candidate && !decompressor.finished()) {
            int written = decompressor.decompress(output, total, candidate - total);
            if (written <= 0) {
              break;
            }
            total += written;
          ...
          // Codecs with no length information (e.g. raw LZ4) fail outright when the output buffer is
          // too small rather than filling it, so treat a failure as a signal to grow. On the last
          // attempt there is nothing left to try, so let it surface.
          if (lastAttempt) {
            throw e;
          }
          undersized = true;
        ...
        // Filling the buffer exactly is also ambiguous: the payload may be complete, or the codec may
        // have had more to write. Grow and retry unless the decompressor confirmed it finished.
        if (!undersized && (total < candidate || decompressor.finished() || lastAttempt)) {
          return BytesInput.from(output, 0, total);
        }
        attemptSize = (long) candidate * 2;
      }
    } finally {
      CodecPool.returnDecompressor(decompressor);
    }
